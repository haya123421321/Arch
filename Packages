...
echo "$password" | sudo -S pacman -S base noto-fonts-emoji virtualbox virtualbox-guest-utils wine lutris spotify-launcher vlc base-devel fontconfig ttf-droid shotwell steam discord bitwarden dolphin binutils linux-headers zsh gcc git make zsh-completions zsh-syntax-highlighting vim --noconfirm

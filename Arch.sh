...
echo "$password" | sudo -S pacman -S base noto-fonts-emoji gimp python-pipx python-pip virtualbox virtualbox-guest-utils wine sqlitebrowser spotify-launcher ttf-dejavu lutris vlc base-devel fontconfig shotwell steam discord bitwarden dolphin binutils linux-headers zsh gcc ntfs-3g git make zsh-autosuggestions zsh-completions zsh-syntax-highlighting vim --noconfirm 
